...
由于流水线处理器中需要进行冒险处理，在EXU检测出控制冒险需要冲刷流水线时，需要将flush信号传递给其他流水段/功能单元，当前方案为在检测出冒险的同周期进行冲刷，增加了关键路径的延迟，使得主频降低为约500MHZ，面积报告如@tbl:timing。后续可以优化为在时钟周期边沿先寄存flush信号，在下一个周期再进行冲刷，从而以寄存器为分割点将关键路径分开，减少关键路径的延迟，从而更好的满足时序逻辑电路建立与保持时间的要求。
...
@tbl:ipc 分别显示了单周期无cache处理器，拥有配置为8行，块大小同字长且直接映射cache的处理器，多周期处理器和流水线处理器的IPC。结合Iron Law，单周期处理器由于所有操作都需要在一个周期内完成，主频较低，所以综合来看其性能较低。表中多周期和流水线数据均为设置在700MHZ下测试，后续将上文主频降低问题修复后即可满足实验设置，故流水线处理器性能较多周期处理器有一定提升。
