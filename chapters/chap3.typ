...
在冯·诺依曼架构计算机中，除了需要包含内部的数据通路和控制部件外，还需要有存储指令和数据的内存，外部大容量持久化存储，以及输入和输出的机制。故而接入外设后处理器才具有执行程序的能力，本章主要介绍与NPC连接的访存模块与SoC环境。

SoC是一种集成电路，将计算机系统的大部分或全部关键组件集成到一个芯片上。通常来说SoC包括一个CPU，配备内存、输入/输出和数据持久化存储功能，以及其他可选功能。

== 访存模块概述

核心内部的ICache和LSU通过访存接口（Memory Interface）进行访存，设备主要包括顶层模块内的CLINT与SoC上的SRAM，Flash，SDRAM和UART，设备的地址空间映射如@tbl:map 所示。

访存模块中包含多个主设备和多个从设备。由@fig:devices 可以看出流水线执行过程中多个单元可能会同时访问一个模块（即多主设备带来的结构冒险），比如通用寄存器组，CSR和内存。所以需要将通用寄存器组和CSR设置为可同时读写，以及使用仲裁器（Arbiter） 对内存请求进行仲裁，优先完成LSU的请求，阻塞ICache。访存模块中也存在多个从设备，所以需要使用交叉开关（Crossbar，Xbar）进行路由，通过组合逻辑将请求直通到对应的从设备来建立访存通路。访存通路的中间模块作为代理主设备向下一个节点发起请求，并在收到响应后作为从设备向上一个节点返回。其中CLINT模块主要用于提供时钟功能，根据处理器主频将计数器的值换算为绝对时间。
