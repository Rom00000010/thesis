...
将NPC接入ysyxSoC#[@ysyxSoC]，其中主要包含了总线与设备部分，其结构如 @fig:soc 所示。由于不同的设备支持的通信模式不同，故而各设备使用了不同的总线接口，其中包括AXI，APB，SPI，wishbone等总线协议，以SPI总线#[@spi_manual]为例，总线传输时序如 @fig:spi 所示，CS_n为片选信号，表示当前从设备被选中，SCK为时钟信号，MOSI和MISO分别为master和slave的输出信号。主设备发起通信时需要先拉低对应从设备的片选信号表示选中，而后给出主设备分频驱动的时钟信号，从设备逐周期接受主设备的命令，在成功接受后的下一个时钟边沿开始返回数据。

ysyxSoC中通过Frag和Yanker对AXI请求进行预处理以便后续将AXI请求转换为其他总线协议对应的请求，AXI4Xbar上连接了SRAM和AXI到APB的转接桥，并将APB-delayer置于Xbar上方以附加延迟，其余设备均挂在APBXbar上。其中SRAM为AXI接口，作为高速存储器，一个周期即可完成读写，UART为APB接口，用于进行串行输出，SDRAM控制器为APB接口，SPI-master为wishbone接口，flash本身为SPI接口。
...
由于ysyxSoC中集成的Flash使用了SPI总线用以减少数据位宽，所以需要作为从设备挂载到SPI-master上。SPI-master#[@spi_master_manual]通过设备寄存器抽象向CPU提供自身支持的功能，其包含的设备寄存器如@tbl:spi_master 所示，在使用过程中，通常需要先设置DIVIDER为用于时钟分频的除数，而后将SS设置为要选择的设备，通过TxX给出要发送的命令，设置CTRL中的各字段以指定数据发送的字节顺序，使用哪一个时钟边沿等参数后启动传输，并在后续轮询SPI-master是否忙碌，在空闲时从RxX读取接受到的数据。
...
根据SPI-master手册的规约与寄存器组织@tbl:spi_master，本文实现的XIP状态机如@fig:xip，在收到请求时分辨是SPI-master的访问还是对Flash的访问，若是对Flash的访问则进入XIP模式，依照上文中描述的SPI-master命令序列与其进行交互。在NPC中实际上只在最初测试SPI-master时会对其进行访问，在实际执行程序中均为通过XIP访问，后续将程序加载到其他存储器后就不再需要访问Flash。
...
若处理器核心和设备的主频比值为n, 当处理器核心在t0时刻发起APB请求，设备在t1时刻响应时，则APBdelayer需要在满足式（2.1）
