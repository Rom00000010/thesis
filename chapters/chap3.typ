...
在处理器实际运行过程中，核心与外设通常运行在不同的频率下，所以为了获得周期级精确的模拟，在仿真环境中需要将APBdelayer模块集成到APBXbar的上游用以捕获所有APB请求，先作为代理master与设备完成通信，在等待对应的延迟时间完成后作为slave将数据返还给实际master。
...
APBdelayer在初始化/空闲状态下，保持主从设备直通。收到访存请求和事务进行中时逐周期累加计数器，最后在从设备响应的时钟边沿动态计算延迟周期。进入等待的延迟时间时，暂时拉低从设备的请求和主设备的响应，直到延迟周期结束重新返回响应并开始接受新的请求。